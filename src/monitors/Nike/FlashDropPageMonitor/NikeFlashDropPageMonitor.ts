...
export default class NikeFlashDropPageMonitor extends Monitor {
  ...
  private lookUpRequest: AxiosRequestHeaders = {
  ...
  private async getPageDataLayerResponse(
    ...
    const wantedUrl2 = 'https://www.nike.com.br/ProductLookup';
    await this.page!.setUserAgent(this.userAgent.random().toString());
    const [req, resDataLayer, resLookup] = await Promise.all([
      this.page!.waitForRequest(
        (httpReq) => httpReq.url().includes(wantedUrl2),
        {
          timeout: 0,
        }
      ),
      ...
      this.page!.waitForResponse(
        (httpRes1) => httpRes1.url().includes(wantedUrl2),
        {
          timeout: 0,
        }
      ),
    ...
    fs.writeFileSync('test.json', JSON.stringify(await this.page?.cookies()));
    return resDataLayer.json();
  }
  ...
  private async getUrls(
    ...
    const url = `https://www.nike.com.br/ProductLookup/${search}`;
    let mappedUrls: { styleCode: string; url: string; imgUrl: string }[] = [];

    const res = await axios.get(url, { headers: this.lookUpRequest });
    console.log(res);
