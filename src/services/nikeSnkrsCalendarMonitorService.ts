...
export class NikeSnkrsCalendarMonitorService {
	...
	private static async _findSnkrs(
	  ...
	  if (this.firstTime) {
	    this.lastLoadedSnkrsUrls = latestTwentySnkrs.map((snkrNode) => this._removeQueryParamsFromUrl(
	  ...
	  const newUrls = latestTwentySnkrs.map((snkrNode) => this._removeQueryParamsFromUrl(
				$(snkrNode).children('a').attr('href')!,
	  ));
