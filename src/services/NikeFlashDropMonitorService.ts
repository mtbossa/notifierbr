...
export class NikeFlashDropsMonitorService {
	...
	public filterOnlyDesiredSneakers(products: Product[]): Product[] {
	  return products
	    ...
	    .filter((product) => product.isAvailable === 1 && product.skus.length > 0); // Removes not availables
	}
	...
	public mapNeededSneakerDataForDiscord(sneaker: Product): SneakerData {
	  ...
	  return {
	    name: sneaker.name,
	    url, // TODO! get url from sku where style is correct
	    imgUrl, // TODO! get url from sku where style is correct
