...
export class NikeFlashDropPageRepository extends NikeFlashDropRepositoryInterface {
  ...
  async getNewSneakersOfThisSearch(requestObject: NikeAPISearchRequest): Promise<SneakerData[]> {
    ...
    do {
      ...
      if (currentPageNewUniqueSneakers && currentPageNewUniqueSneakers.length > 0) {
        const mappedCurrentPageNewUniqueSneakers: SneakerData[] = currentPageNewUniqueSneakers.map((sneaker: Product) => this._nikeFlashDropMonitorService.mapNeededSneakerDataForDiscord(sneaker));
